...
A renderização de Holographic permite que seu aplicativo desenhe um holograma em um local preciso no mundo todo, seja precisamente colocado no mundo físico ou em um realm que você criou. Os [hologramas](../../discover/hologram.md) são objetos compostos de som e luz. A renderização permite que seu aplicativo adicione a luz.
...
Consulte o artigo [renderizando no DirectX](../native/rendering-in-directx.md) para obter detalhes de baixo nível sobre seu tópico.
