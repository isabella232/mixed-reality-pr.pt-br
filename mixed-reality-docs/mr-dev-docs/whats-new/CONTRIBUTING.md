...
Esta página aborda as etapas e diretrizes básicas de contribuição, bem como links para noções básicas de redução. Agradecemos a sua contribuição.
...
- O `redirect_url` é a URL pública relativa do artigo antigo para o novo artigo. Certifique-se de que **essa URL não** contém `mixed-reality-docs` ou `.md` , pois se refere à URL pública e não ao caminho do repositório. É permitido vincular a uma seção dentro do novo artigo usando `#section` . Você também pode usar um caminho absoluto para outro site aqui, se necessário.
