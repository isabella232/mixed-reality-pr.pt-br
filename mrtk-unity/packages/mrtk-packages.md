...
O MRTK (Mixed Reality Toolkit) é uma coleção de pacotes que permitem o desenvolvimento de aplicativos de Realidade Misturada entre plataformas, fornecendo suporte para plataformas e hardware de Realidade Misturada.
...
Esses pacotes são lançados e têm suporte da Microsoft do código-fonte [no branch mrtk_release](https://github.com/Microsoft/MixedRealityToolkit-Unity/tree/mrtk_release) no GitHub.

### <a name="foundation-package"></a>Pacote foundation

O Mixed Reality Toolkit Foundation é o conjunto de códigos que permite que seu aplicativo aproveite a funcionalidade comum nas Plataformas de Realidade Misturada.
...
| | Windows | Suporte para APIs Windows Microsoft, por exemplo, fala e ditado. |
...
| | Perfis | Perfis padrão para os serviços e sistemas Toolkit Microsoft Mixed Reality. |
...
| MRTK/StandardAssets | | Sombreador Padrão do MRTK, materiais básicos e outros ativos padrão para experiências de realidade misturada |
...
O pacote opcional Microsoft. MixedRealityToolkit. Unity. Tools inclui ferramentas úteis que aprimoram a experiência de desenvolvimento da realidade misturada usando o Toolkit de realidade misturada da Microsoft.
essas ferramentas estão localizadas na **realidade misturada Toolkit menu > Utilities** no Editor do Unity.
...
O pacote opcional Microsoft. MixedRealityToolkit. TestUtilities é uma coleção de scripts auxiliares que permite aos desenvolvedores [criar facilmente testes de modo de reprodução](../contributing/unit-tests.md#play-mode-tests). Esses utilitários são especialmente úteis para desenvolvedores que criam componentes MRTK.
...
- Projetos menores
  - soluções de Visual Studio de limpeza
...
- MRTK não dá suporte a pacotes UPM com Unity 2018,4

### <a name="foundation-package"></a>Pacote do Foundation

O pacote Foundation ( `com.microsoft.mixedreality.toolkit.foundation` ) constitui a base da realidade misturada Toolkit.
...
> [!Note]
> Para evitar alterações significativas com base nas definições de assembly, os scripts usados para controlar alguns recursos do sombreador padrão do MRTK não estão incluídos no pacote de ativos padrão. Esses scripts podem ser encontrados no pacote de base na `MRTK/Core/Utilities/StandardShader` pasta .
...
| | LostTrackingService | Serviço que simplifica a entrega da perda de acompanhamento em Microsoft HoloLens dispositivos. |
| | [SceneTransitionService](../features/extensions/scene-transition-service.md) | Serviço que simplifica a adição de transições de cena suaves. |
| | Exemplos~ | Uma pasta oculta (no Editor do Unity) que contém as cenas de exemplo e os ativos. |

Mais detalhes sobre o processo de uso de pacotes que contêm projetos de exemplo podem ser encontrados no artigo Toolkit e [unity Gerenciador de Pacotes.](../configuration/usingupm.md#using-mixed-reality-toolkit-examples)
...
O pacote de ferramentas opcionais ( `com.microsoft.mixedreality.toolkit.tools)` contém ferramentas úteis para criar experiências de realidade misturada. Em geral, essas ferramentas são componentes do editor e seu código não é ship como parte de um aplicativo.
...
O pacote de utilitários de teste opcional ( ) contém uma coleção de scripts auxiliares que permitem aos desenvolvedores criar facilmente `com.microsoft.mixedreality.toolkit.testutilities` testes de modo de reprodução. Esses utilitários são especialmente úteis para desenvolvedores que criam componentes do MRTK.
