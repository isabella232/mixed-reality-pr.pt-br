...
A cena funciona no modo de jogo do Unity e no dispositivo. Execute a cena **MRTKExamplesHub** no editor do Unity e use a simulação de entrada do MRTK para interagir com o conteúdo da cena. Para compilar e implantar, basta criar uma cena **MRTKExamplesHub** com outras cenas incluídas na lista do sistema de cena. O Inspetor também facilita a adição de cenas às configurações de compilação. Nas configurações de compilação, verifique se a cena **MRTKExamplesHub** está na parte superior da lista no índice 0.
